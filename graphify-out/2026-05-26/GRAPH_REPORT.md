...
## Corpus Check
- Corpus is ~41,751 words - fits in a single context window. You may not need a graph.

## Summary
- 488 nodes · 620 edges · 44 communities (28 shown, 16 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 32 edges (avg confidence: 0.86)
...
3. `request()` - 14 edges
4. `useToast()` - 9 edges
5. `useRoomContext()` - 9 edges
6. `Room CRUD Routes` - 9 edges
7. `Message` - 7 edges
8. `RoomSocket` - 7 edges
9. `scripts` - 7 edges
10. `getDb()` - 7 edges
...
Cohesion: 0.07
Nodes (36): ChatHeader(), ChatHeaderProps, Room expiry countdown with live 1s update interval, ChatPage(), ChatPage orchestrates crypto, presence, message, connection hooks, E2EE key derivation from room password, MessageInput(), MessageInputProps (+28 more)

### Community 1 - "API Validation Schemas"
Cohesion: 0.06
Nodes (36): generatePresignedPutUrl(), getSignatureKey(), hmacSha256(), PresignedUrlOptions, createRoomSchema, joinRoomSchema, leaveRoomSchema, messagesQuerySchema (+28 more)

### Community 2 - "Lobby & API Client"
Cohesion: 0.10
Nodes (23): useTheme(), ApiError, createRoom(), CreateRoomResponse, getMessages(), joinRoom(), JoinRoomResponse, leaveRoom API (+15 more)
...
Cohesion: 0.11
Nodes (24): Drizzle ORM Data Layer, In-Memory State (WS+RateLimit), R2 Presigned Upload Pattern, Hono Mounted Routes Pattern, Test Infra (Bun Shim+File SQLite), Invite+Session Token Auth, WS Room System (In-Memory State), Drizzle ORM Config (+16 more)
...
Cohesion: 0.16
Nodes (17): getDb(), messages, rooms, roomTokens, requireSession(), close(), message(), broadcastToRoom() (+9 more)
...
Cohesion: 0.18
Nodes (17): VanisChat Agents Guide, GitHub Actions CI/CD, arrayBufferToBase64(), base64ToArrayBuffer(), decrypt(), deriveKey(), encrypt(), generateSalt() (+9 more)

### Community 12 - "App Shell & Errors"
Cohesion: 0.13
Nodes (11): ErrorBoundary, ErrorBoundaryProps, ErrorBoundaryState, ErrorFallback(), ErrorFallbackProps, RoomErrorFallback(), defaultState, RoomContext (+3 more)
...
## Knowledge Gaps
- **249 isolated node(s):** `extends`, `name`, `version`, `private`, `workspaces` (+244 more)
...
- **Why does `request()` connect `Lobby & API Client` to `Message Display`, `React Providers`?**
